...
2. In SFP, verify every parameter against [snapshot §2](ymmgrnnote.md#2-exact-legacy-interface-contract), add the four exact legacy exceptions using SFP's native UI, and inspect `LT_MSEG`/`LS_MSEG` component DDIC references. Configure every actual `QUAN` with its DDIC unit target and every actual `CURR` with its DDIC currency target; do not infer those targets from names. Activate interface and form, then manually drag/drop each required node from the left Interface tree into the right Context tree and verify the mapped nodes beneath the Context root. Stage -> Commit -> Push the generated SFPF/SFPI. No bound XDP element or table may be added before that captured Context exists.
